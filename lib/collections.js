...
Meteor.methods({
  'othello.updateGameData'({ gameId, changedGameData, currentPlayerTurn }) {
    ...
    if (currentPlayerTurn == 0){currentPlayerTurn = 1;}
      ...
      $addToSet: { turnData: {date: Date.now(), turnMove: changedGameData, playerTurn: currentPlayerTurn}}});
  	...
  	'othello.readGameData'({gameId}) {
  		var readGameData = PieceCollection.findOne({_id: gameId}, {_id: 0, turnData: 1 });
  		return readGameData;
  	},
  	'othello.resetGameData' ({gameId}) {
  		var initialData = { //Initial starting positions in object literal notation
   		...
   		PieceCollection.update({_id: gameId}, {
  			...
  			$set: { turnData: [{date: Date.now(), turnMove: initialData, playerTurn: 0}], gameStart: Date.now(), gameEnd: null }}, {upsert: true});
