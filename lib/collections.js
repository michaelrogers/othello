...
Meteor.methods({
  ...
  //      	}
    ...
    var initialData = [row0,row1,row2,row3,row4,row5,row6,row7];
   		PieceCollection.update({_id: gameId}, {
    ...
    'othello.chatMessageInsert' ({gameId,username,message}){
      var currentGame = PieceCollection.findOne({_id: gameId});
      if (currentGame['playerWhite'] == username || currentGame['playerBlack'] == username){
        ChatMessages.insert({
              gameId: gameId,
              username: username,
              message: message,
      ...
      else {throw new Meteor.Error(500, 'Invalid Player', 'Player cannot chat about game if they are not a participant')}
